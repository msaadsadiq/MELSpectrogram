...
def audioframes2logmelspec(b_framed_signal, n_ffts=1, wvls_per_window_hinge=16, n_mel=128, fft_l1=1024, sr=48000):
  ...
  print(magnitude_spectros.shape[2])
...
def plot_several_logmelspec(paths):
  ...
  for i, path in enumerate(paths):
    ...
    librosa.display.specshow(log_mel_spectro[0,:,:,0].numpy().T, sr=sr, x_axis='time', y_axis='mel', hop_length=32, fmin=tf.reduce_min(center_mel_freqs), fmax=tf.reduce_max(center_mel_freqs), cmap='coolwarm')
